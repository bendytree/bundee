...
module.exports = function(bundeeApp, options, type){
  ...
  var beginCache = function(key, srcs){
    ...
    for(var i=0; i<srcs.length; i++){
      ...
      if(!/^http/i.test(url)){
        //add the base URL
        url = getOption("baseUrl")+url.replace(/^[~\/]*/, '');
      }
  ...
  var route = getOption('bundlePath').replace(':type', type);

  //`app.get` registers routing too early, so create our own app and `use` it
  bundeeApp.get(route, routeHandler);
